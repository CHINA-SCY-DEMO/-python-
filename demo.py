                ...
                if submitted:
                    if len(new_password) < 6:
                        ...
                        if success:
                            ...
                            st.error(msg)
         # ========== 新增：隐私声明（符合《个人信息保护法》） ==========
        with st.expander("📜 隐私声明与数据使用政策", expanded=False):
            st.markdown("""
            **数据采集与使用**  
            - 本系统仅收集您主动上传的简历文本及注册信息（用户名、邮箱）。  
            - 简历数据仅用于**AI解析、评分与优化**，绝不用于模型训练或向第三方共享。  
            - 调用DeepSeek API时，数据经加密传输且服务商承诺不反哺训练。  

            **数据存储与安全**  
            - 密码采用SHA-256哈希存储；简历内容在数据库中以加密形式存放。  
            - 所有传输强制HTTPS，数据库启用行级安全隔离，用户仅能访问本人数据。  

            **您的权利**  
            - 您可随时在“高级操作”中**一键删除全部历史数据**。  
            - 账户注销后，关联简历与优化记录将**级联彻底删除**。  

            **合规依据**  
            - 本系统设计遵循《中华人民共和国个人信息保护法》第六条（目的限制）、第十六条（删除权）、第二十一条（委托处理）。  
            """)
        # ========================================================
